...
class DatabaseForLibrary(var context: Context) : SQLiteOpenHelper(context, DATABASE_NAME, null, 1) {
    ...
    fun insertReservation(name : String, surname : String, startTime : String, finishTime : String, phone :String, bookId: String) {
        val db = this.writableDatabase
        val book : Book
        val values = ContentValues()
    ...
    fun viewAllReservation(context: Context): ArrayList<Reservation> {
        val db = this.readableDatabase
        val query = "SELECT * FROM " + TABLE_RESERVATION
        val data: Cursor = db.rawQuery(query, null)
        ...
        else
        {
            ...
            while (!data.isAfterLast())
            {
                ...
                reservation.reservationID = data.getInt(data.getColumnIndex("reservationID"))
                reservation.Name = data.getString(data.getColumnIndex("FIRSTNAME"))
                reservation.Surname = data.getString(data.getColumnIndex("SECONDNAME"))
                reservation.StartTime = data.getString(data.getColumnIndex("StartTime"))
                reservation.FinishTime = data.getString(data.getColumnIndex("FinishTime"))
                reservation.Phone = data.getInt(data.getColumnIndex("PHONE"))
                reservation.Book = data.getInt(data.getColumnIndex("ID"))
    ...
    fun deleteReservation(resID :Int):Boolean {
        val query = "Delete From $TABLE_RESERVATION where $RESERVATION_ID = $resID"
        val db = this.writableDatabase
        var result : Boolean = false
        try
        {
